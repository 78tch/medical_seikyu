...
2. 「支払基金のホームページ」
3. 「オンライン利用申請」から数日で、フレッツ回線が登録され、IP-VPN閉域網に接続できるようになる。  
4. オンラインシステム用のパソコンを用意し、「IP-VPN閉域網」に接続するためのネットワーク設定をする。指定のユーザーを作成してそれでOSにログインし、プロキシサーバーやDNSサーバーを設定する。PPPoEの設定で、「ネットワークID」と「ネットワークパスワード」が必要。８桁@tk６桁、８桁。
5. 設定したプロキシサーバーを有効にしている間は、一般のインターネットにはつながらなくなるので、普段はプロキシを無効にしておき、オンラインシステムを使用する時だけ切り替えてプロキシを有効にするか、もしくはオンラインシステム専用のパソコンを用意するか、どちらかとなる。  
6. 「電子証明書発行申請」から数日で、簡易書留で「電子証明書取得に関する情報」が送られてくる。これを使って、オンライン用パソコンで「●④電子証明書等ダウンロードサイト」から電子証明書をダウンロードする。ダウンロードしたら、オンライン用パソコンにインストールする。これで、「●⑥オンライン資格確認等システム」などに接続できるようになる。  https://hweb.oqs.onshikaku.org/web/  
7. 「オンライン利用申請」から数日で、「①医療機関等向け総合ポータルサイト」から、「マスタアカウント情報」がダウンロードできるようになる。オンライン用パソコンで「⑥オンライン資格確認等システム」に接続し、この情報でログインする。  
8. 「⑥オンライン資格確認等システム」にログインしたら、「管理者アカウント」、「顔認証用アカウント」、「一般利用者アカウント」を作成する。  
9. 「②医療機関等ONS」にログインできるように、申請をする。  
10. 「②医療機関等ONS」にログインし、ブラウザ拡張プラグイン（顔認証ライブラリ含む）などをダウンロードし、そのなかに含まれる「自己署名証明書」をインストールする。  
11. 「③カードリーダーのベンダーのホームページ」で、「カードリーダーのマニュアル類」７冊と、「カードリーダーのソフト（９８６MB）」をダウンロードする。「設定画面：ABCDAB」「管理者：123456」「利用者654321」
12. オンライン用パソコンに、マイナンバーカードのカードリーダーをUSB接続し、カードリーダーのソフトをインストールする。
13. 「⑤支払基金のホームページ」から、「オンライン請求」の資料をダウンロードする。このなかに「自己署名証明書」があるので、インストールする。  https://www.ssk.or.jp/seikyushiharai/online/index.html  
14. 「④電子証明書等ダウンロードサイト」から、オンライン請求のソフトをダウンロードする。
