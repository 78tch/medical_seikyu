...
|2|電子証明書|IP-VPN上の「証明書等ダウンロードサイト等」からさらに「証明書ダウンロードサイト」へ移動し、申請により発行された電子証明書をダウンロードする。「リクエストID」と「リファレンスID」が必要。併せて数字４桁の「証明書パスワード」を設定し、それをインストール時に入力する。これによりシステムにアクセスできるようになる。|
