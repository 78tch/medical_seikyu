...
2. オンラインシステム用のパソコンを用意し、「IP-VPN閉域網」に接続するためのネットワーク設定をする。プロキシサーバーやDNSサーバーを設定するが、設定したプロキシサーバーを有効にしていると、一般のインターネットにはつながらなくなるので、オンラインシステムを使用する時だけ切り替えるか、オンラインシステム専用のパソコンを用意するか、どちらかとなる。
3. 「電子証明書発行申請」から数日で、簡易書留で「電子証明書取得に関する情報」が送られてくる。これを使って、「④電子証明書等ダウンロードサイト」で電子証明書をダウンロードする。
4. 「オンライン利用申請」から数日で、ポータルサイトから、「マスタアカウント情報」がダウンロードできるようになる。これを使って「⑥オンライン資格確認等システム」にログインする。
5. 「オンライン処理用パソコン」を用意する。電子証明書をインストールしたり、IP-VPNの設定をしたりして、通常のインターネットとは切り替えて使う必要があるので、通常のパソコンとは別に専用のパソコンを用意したほうがよい。  
