...
|No|項目|内容|
|--|--|--|
|1|電子証明書発行申請|オンライン資格確認システムやオンライン請求システムのサイトにアクセスするのに必要になる。IP-VPN上の「電子証明書ダウンロードサイト等」からDLできるようになる。ダウンロード時に数字４桁の「証明書パスワード」を設定するが、これは証明書のインストール時に入力を求められるので、メモしておくこと。|
...
|3|請求セットアッププログラム|IP-VPN上の「証明書等ダウンロードサイト等」から「OnlineSetup_Iryou_Windows.zip」をDLする。この中にも「自己署名証明書」が入っているが、通常は使わないであろう。併せてAdobeReaderDCと操作マニュアルもDLする。|
|4|資格確認システムセットアッププログラム|必要に応じて、インターネット上の「医療機関等ONS」から、カードリーダーのドライバー「PFU-CaoraApp_v1.8.2.1」か顔認証アプリなどをダウンロードする。|
