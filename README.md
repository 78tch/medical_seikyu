...
## 流れ
1. 「オンライン処理用パソコン」を用意する。
2. 「医療機関等向け総合ポータルサイト」にログインできるようにする。  
3. インターネット回線の「お客さまID」を登録する。なお、「お客さまID」は、ONU に貼ってあるシールにも書いてある。  
4. 「オンライン処理用パソコン」で、登録した「お客さまID」と、交付された「ネットワーク ID」と「ネットワークパスワード」で、IP-VPN接続する。  
5. 「オンライン処理用パソコン」に、支払基金のホームページからダウンロードできる「自己署名証明書」をインストールする。これで、IP-VPN 網 のなかにある「電子証明書ダウンロードサイト」に接続できるようになる  
6. 「オンライン処理用パソコン」に、IP-VPN 網のDNS の設定をする。各種サーバーに接続できるようになる。  
...
8. 「電子証明書ダウンロードサイト」から、「オンライン請求」のセットアッププログラムをダウンロードして、インストールする。  
...
5. 連携アプリケーションは医療機関等ベンダーから「OqsInstall.exe 」を入手する。
